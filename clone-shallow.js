...
let cartClone = [...cart];
let hatClone = {...hatProduct};

const addProduct = function (cartClone, hatClone) {
    return [...cartClone,hatClone];
}
const newCart = addProduct(cartClone,hatClone);
console.log(newCart);
console.log(cart);
...
const finishTask = function(taskClone){
    taskClone.pop();
    return taskClone;
}
const newArray = finishTask(taskClone);
...
clonePerson["fullName"];

 const newProp = function(clonePerson){
     clonePerson.fullName = `${clonePerson.firstName} ${clonePerson.lastName} `
     return clonePerson;
 }
 const result = newProp(clonePerson);
 console.log(person);
...
function applyDiscount (products){
    return products.map(cloneProduct => {
        let discount;
        if (cloneProduct.type === "t-shirt") discount = 20;
        else if (cloneProduct.type === "jeans") discount = 10;
        else if (cloneProduct.type === "shirt") discount = 15;

        const discountedPrice = cloneProduct.price - (cloneProduct.price * discount / 100);

        return {...cloneProduct,discount,price: discountedPrice};

    }); 
} 
const result1 = applyDiscount(products);
console.log(result1);
